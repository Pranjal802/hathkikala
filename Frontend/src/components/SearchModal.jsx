    ...
    <AnimatePresence>
      {searchOpen && (
        <>
          ...
          >
            <div className="bg-[#FFF8F2] rounded-3xl shadow-2xl overflow-hidden border border-rose-100">
              {/* Search bar */}
              ...
              <div className="max-h-80 overflow-y-auto">
                {query.trim().length < 2 ? (
                  <div className="px-5 py-8 text-center">
                    ...
                    <p className="font-sans text-sm text-[#5C4033]/60">Type to search handmade products...</p>
                    <div className="flex flex-wrap gap-2 justify-center mt-4">
                      {['Crochet', 'Slime', 'Mirror Gloves', 'Clutch'].map(tag => (
                        <button
                          key={tag}
                          onClick={() => setQuery(tag)}
                          className="font-sans text-xs bg-[#F5E6DA] text-[#5C4033] px-3 py-1.5 rounded-full hover:bg-[#D8A7B1] hover:text-white transition-all font-semibold"
                        >
                          {tag}
                        </button>
                      ))}
                  ...
                  <div className="px-5 py-8 text-center">
                    ...
                    <p className="font-sans text-sm text-[#5C4033]/60">No products found matching "<strong>{query}</strong>"</p>
                  </div>
                ) : (
                    ...
                    {results.map(product => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="font-sans text-xs text-[#5C4033]/50 line-clamp-1">{product.description}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-sans text-sm font-extrabold text-[#C97C5D]">₹{product.discountPrice || product.basePrice}</p>
                          <button
                            onClick={() => { addToCart(product); setSearchOpen(false); setQuery(''); }}
                            className="font-sans text-xs text-[#9CAF88] hover:text-[#C97C5D] font-bold transition-colors"
                          >
                            + Add to Cart
                          </button>
                        </div>
